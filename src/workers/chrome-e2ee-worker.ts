/**
 * @module workers/chrome-e2ee-worker
 * E2EE transform worker for Chrome/Chromium using Insertable Streams API
 *
 * @description
 * This worker handles real-time encryption/decryption of WebRTC media frames
 * using the Insertable Streams API (RTCRtpSender/Receiver.createEncodedStreams).
 *
 * Usage:
 * 1. Create worker: `new Worker('chrome-e2ee-worker.js', { type: 'module' })`
 * 2. Send 'init' message with config
 * 3. Send 'set-key' message with encryption key
 * 4. Pipe encoded streams through worker using transferable streams
 *
...
/** Message with streams for Insertable Streams API */
...
/**
 * Creates an encryption transform for outgoing frames
 *
 * @returns TransformStream that encrypts frames
 */
function createEncryptTransform(): TransformStream<
...
/**
 * Creates a decryption transform for incoming frames
 *
 * @returns TransformStream that decrypts frames
 */
function createDecryptTransform(): TransformStream<
  RTCEncodedVideoFrame | RTCEncodedAudioFrame,
  RTCEncodedVideoFrame | RTCEncodedAudioFrame
> {
  return new TransformStream({
 ...
 */
function isStreamMessage(data: unknown): data is StreamMessage {
  return typeof data === 'object' && data !== null && 'readable' in data && 'writable' in data;
}
...
self.onmessage = async (event: MessageEvent): Promise<void> => {
  ...
  // Handle RTCEncodedVideoFrame/RTCEncodedAudioFrame streams (Insertable Streams API)
  if (isStreamMessage(data)) {
    const readable = data.readable as ReadableStream<RTCEncodedVideoFrame | RTCEncodedAudioFrame>;
    const writable = data.writable as WritableStream<RTCEncodedVideoFrame | RTCEncodedAudioFrame>;
    const transform =
      config?.mode === 'encrypt' ? createEncryptTransform() : createDecryptTransform();
